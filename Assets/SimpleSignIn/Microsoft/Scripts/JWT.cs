...
using System.Text;
using Assets.SimpleSignIn.Microsoft.Scripts.Utils;
...
namespace Assets.SimpleSignIn.Microsoft.Scripts
{
    ...
    public class JWT
    {
        ...
        /// <summary>
        /// More info: https://learn.microsoft.com/en-us/entra/identity-platform/id-tokens#validate-tokens
        ...
        public void ValidateSignature(string clientId, Action<bool, string> callback)
        {
            ...
            {
                if ((string) payload["iss"] != knownPublicKeys[kid]["issuer"])
                {
                    callback(false, "Unexpected payload (iss).");
                    return;
                }
            ...
            request.SendWebRequest().completed += obj =>
            {
                if (request.result == UnityWebRequest.Result.Success)
                {
                    ...
                    var keys = certs["keys"].ToDictionary(i => i["kid"].Value<string>(), i => new Dictionary<string, string>
                    {
                        { "n", i["n"].Value<string>() },
                        { "e", i["e"].Value<string>() } ,
                        { "issuer", i["issuer"].Value<string>() }
                    });
