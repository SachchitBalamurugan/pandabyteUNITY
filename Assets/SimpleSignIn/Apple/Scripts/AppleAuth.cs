...
namespace Assets.SimpleSignIn.Apple.Scripts
{
    ...
    public partial class AppleAuth
    {
        ...
        private string _redirectUri, _state, _nonce;
        ...
        private void Auth()
        {
            ...
            var authorizationRequest = $"{AuthorizationEndpoint}?client_id={_settings.ClientId}&nonce={Helpers.CreateSha256Hex(_nonce)}&redirect_uri={Uri.EscapeDataString(redirectUri)}&response_mode=form_post&response_type=code&scope={Uri.EscapeDataString(string.Join(" ", _settings.AccessScopes))}&state={_state}";
