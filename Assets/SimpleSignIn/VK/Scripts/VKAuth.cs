...
namespace Assets.SimpleSignIn.VK.Scripts
{
    ...
    public class VKAuth : MonoBehaviour
    {
        ...
        public static void SignIn(Action<bool, string, UserInfo> callback)
        {
            ...
            _redirectUri = $"{VKAuthSettings.Instance.CustomUriScheme}:/oauth2callback/vk";
        ...
        private static void Auth()
        {
            ...
            request.SendWebRequest().completed += obj =>
            {
                if (request.result == UnityWebRequest.Result.Success)
                {
                    var authorizationRequest = $"{AuthorizationEndpoint}?client_id={VKAuthSettings.Instance.ClientId}&redirect_uri={Uri.EscapeDataString(AuthorizationMiddleware + "/redirect")}&display=page&scope={AccessScope}&response_type=code&state={_state}";
        ...
        private static void UseSavedToken()
        {
            ...
            if (SavedAuth.ClientId == VKAuthSettings.Instance.ClientId && SavedAuth.ExpirationTime > DateTime.UtcNow)
            {
        ...
        {
            var request = UnityWebRequest.Get($"{TokenEndpoint}?client_id={VKAuthSettings.Instance.ClientId}&client_secret={VKAuthSettings.Instance.ClientSecret}&redirect_uri={Uri.EscapeDataString(AuthorizationMiddleware + "/redirect")}&code={code}");
            ...
            request.SendWebRequest().completed += _ =>
            {
                if (request.error == null)
                {
                    ...
                    SavedAuth = new SavedAuth(VKAuthSettings.Instance.ClientId, TokenResponse);
